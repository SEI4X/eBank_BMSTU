...
//MARK: - Allert to confirm deleting
    func agreeToTerms(indexPath: IndexPath) {
        ...
        let defaultAction = UIAlertAction(title: "Закрыть", style: .default) { (action) in
//            self.members.remove(at: indexPath.row)
 ...
 //           self.myTableView.deleteRows(at: [indexPath], with: .automatic)
       ...
       let alert = UIAlertController(title: "Закрытие счета",
             message: "Вы уверены, что хотите закрыть счет? Данное действие нельзя будет отменить.",
